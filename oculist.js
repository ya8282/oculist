(function () {
  ...
  function animateLightning(rect) {
    ...
    var merge = document.createElementNS('FeMerge', 'feMerge');
    if (!merge || typeof merge.setAttribute !== 'function') {
      merge = document.createElementNS('http://www.w3.org/2000/svg', 'feMerge');
    }
  ...
  function buildSettingsPanel() {
    ...
    var items = [
      ...
      { label: i18n.beaconLabel, val: effColors.beacon, title: i18n.beaconTitle, cb: function (v) { settings.beaconColor = v; saveSettings(); } }
    ];
  ...
  if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.sync) {
    chrome.storage.sync.get('oc-settings', function (data) {
      if (data && data['oc-settings']) {
        ...
        ['effect', 'position', 'theme', 'matchColor', 'activeColor', 'beaconColor', 'scrollBehavior', 'disabledSites', 'performanceMode', 'visionProfile', 'visionSettings', 'setupWizardCompleted'].forEach(function (k) {
