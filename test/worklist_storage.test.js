...
describe('Working-list session storage (oc-worklist)', () => {
  ...
  before(async () => {
    ...
    // This suite's own raw chrome.storage.session calls (unlike content.js's own
    // loadWorkList()/saveWorkList(), which already retry once) have no protection of their
    // own against background.js's setAccessLevel() startup race — see
    // helpers/session_access.js for the full account, including the oculist-z4s repro this
